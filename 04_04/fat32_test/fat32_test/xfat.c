...
xfat_err_t xdir_first_file(xfile_t* file, xfileinfo_t* info)//传入file结构, 还有传地址取值的info
{
	...
	//获取dir_item
	err = locate_file_dir_item(file->xfat, &file->curr_cluster, &cluster_offset, "", &moved_bytes, &diritem);	//文件路径: 设置"", 为空, 也就是说, 我们可以通过xfat.c中的if(path == (const char*)0), 也就是找到任意个有效的目录项, 不管是文件还是目录都返回 //找到diritem后, 在簇链中移动了moved_bytes
	if(err < 0) return err;
