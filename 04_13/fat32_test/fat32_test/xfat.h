...
//4.7
xfat_err_t xfile_error(xfile_t* file); //获取错误码
void xfile_clear_err(xfile_t* file); //清除错误码
...
//4.9 
xfat_err_t xfile_eof(xfile_t* file);//判断文件的当前的读写位置是否到了文件结束
xfile_size_t xfile_tell(xfile_t* file); //获取文件当前的位置
xfat_err_t xfile_seek(xfile_t* file, xfile_ssize_t offset, xfile_origin_t origin);//文件读写位置的定位

//4.10	修改文件名
xfat_err_t xfile_rename(xfat_t* xfat, const char* path, const char* new_name);

//4.11 修改文件的access time, modify time, create time
