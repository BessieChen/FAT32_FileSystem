...
int disk_io_test(void) {
	...
	//1.3 改, 调用驱动的打开接口
	err = xdisk_open(&disk_test, "vdisk_test", &vdisk_driver, (void*)disk_path);//加入了名字, 还有vdisk_driver的路径 //1.3 删err = disk_test.driver->open(&disk_test, disk_path_test);
...
int main(void)
{
	//write缓冲的初始化 //bug呀!你的disk_io_test()都把内容给覆盖了
	for (int i = 0; i < sizeof(write_buffer) / sizeof(u32_t); i++) {
		write_buffer[i] = i;
	}

	xfat_err_t err;
	//bug呀!你的disk_io_test()都把内容给覆盖了
