...
typedef struct _mbr_part_t {
	...
	u32_t relative_sectors; //相对的扇区数: 在一个fat32分区中, 某个扇区举例该分区的第一个扇区的相对位置
...
//2.4 结构体: 存放fat32分区的相关信息, 这里说的fat32分区指的是mbr中4个分区配置所指向的4个分区之一, fat32分区里面分别有:(保留区, fat区, 数据区)
typedef struct _xdisk_part_t {
	u32_t start_sector;//(也就是保留区的位置) 起始扇区号, 相对于disk起始物理地址
	u32_t total_sector;//占了多少扇区
	xfs_type_t type;//分区类型
	xdisk_t* disk;//分区所在的disk
}xdisk_part_t;
