...
int main(void)
{
	...
	//3.1 读取FAT32的第0块扇区(保留区), 保存到read_buffer中, 然后将read_buffer看成一个dbr_t,这样就可以知道保存区里面的信息
	err = xdisk_read_sector(&disk, (u8_t*)read_buffer, disk_part.start_sector, 1); //disk_part.start_sector保存指向的FAT32部分的起始地址. 第0块扇区(保留区)

	//3.1 将read_buffer转换成我们的dbr_t结构, 然后调试看内部
